...
class CNexify:
    ...
    def writeFile( self ):
            ...
            if ( obj.getType() == 'Mesh' ):
                ...
                for prop in propList:
                    ...
                    if prop_name == 'ignore':
                        ...
                        # it's entity of different then worldspawn kind.
                        classname = prop.getData()
                        # remember classname and obj.
                ...
                if ( skipObject ):
                    self.nprint( "skipping this object due to 'ignore' property" )
                    continue
                worldspawn_objs.append( obj )
                # self.writeMesh( file, obj )
                
        # Appending worldspawn values.
        stri = ""
        n = len( keys )
        for i in range( n ):
            stri += '    "%s" "%s"\n' % ( keys[i], vals[i] )
        ...
        # Writing worldspawn geometry
        for obj in worldspawn_objs:
            self.writeMesh( file, obj )
    ...
    def writeMesh( self, file, obj ):
        btMesh = nex_btreemesh.CBTreeMesh( self )
