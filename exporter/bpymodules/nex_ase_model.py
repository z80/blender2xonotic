"""
This file is a part of Nexuiz MAP exporter.
It's aim is export models in ASE format for further 
inserting them into MAP file as "misc_model" entity type.
Based on "ase_export.py". And key point is it exports 
just one mesh selected by it's name.
"""

import Blender, time, math, sys as osSys
from Blender import sys, Window, Draw, Scene, Mesh, Material, Texture, Image, Mathutils
import os
from os import path

class AseWriter():
   def __init__( self, file_name, mesh_name ):
...
def g_write_ui( filename ):
    ase = AseWriter( "Box" )
    ase.write_ui( filename )
    del ase
    
    
if __name__ == '__main__':
    Window.FileSelector(g_write_ui, 'Export ASCII Scene', sys.makename(ext='.ase'))
